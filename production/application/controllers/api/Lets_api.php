...
/***Tested Till Here***/
    ...
    public function get_sight_location_post(){
  		...
  		$result = $this->Api_model->get_sight_location();
  		if($result->num_rows()){
  		  $data = $result->result_array();
  		  $setting['success'] = '1';
  		  $setting['message'] = "Data Found.";
    ...
    public function add_outstation_post(){
      ...
      $id = $this->Api_model->isAccessId($access_token);
      if($id){
      $result = $this->Api_model->add_outstation($id);
      if($result){
        $setting['success'] = '1';
        $setting['message'] = "Your outstatio trip request sent successfully. We will confirm you soon.";
        $this->response(array('setting' => $setting));
      }
      else{
        $setting['success'] = '0';
