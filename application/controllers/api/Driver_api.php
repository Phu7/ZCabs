...
class Driver_api extends REST_Controller {
	...
	function edit_profile_post(){
		...
		if($id){
			...
			$query = $this->Driver_api_model->edit_driver($dp, $id);
			if($query){
				$setting['success'] = '1';
	...
	function manage_documents_post(){
		if($this->input->post('api_key') != '54st65h4esrth14s3ghd5f4g3d5fg13d2fg1d32fy4d2fg1hd3'){
			$setting['success'] = '2';
			$setting['message'] = "Api key is not valid";
			$this->response(array('setting' => $setting));
		...
		} elseif($this->input->post('access_token')){
			$user_id = $this->Api_model->isAccessId($this->input->post('access_token'));
		} else{
			$setting['success'] = '2';
			$setting['message'] = "No User found.";
			$this->response(array('setting' => $setting));
			exit();
		}
		
    	$query = $this->Driver_api_model->manage_document($user_id);
		if($query){
			$setting['success'] = '1';
			$setting['message'] = "Successfully registered. Approval is on process.";
			$this->response(array('setting' => $setting));
	...
	function manage_duty_post(){
		...
		if($this->input->post('status') == ""){
			...
			if($query){
				$setting['success'] = '1';
				$setting['message'] = "You are now " . $st . ".";
				$this->response(array('setting' => $setting));
			}
			else{
				$setting['success'] = '0';
