...
class Common extends CI_Controller {
	...
	public function index(){
		if($this->admin->isLogged()){
			$data['total_users'] = $this->db->query("SELECT COUNT(id) as total FROM customer")->row()->total;
			$data['users'] = $this->db->query("SELECT COUNT(id) as total FROM customer WHERE role_id = 1")->row()->total;
			$data['drivers'] = $this->db->query("SELECT COUNT(id) as total FROM customer WHERE role_id = 2")->row()->total;
			$data['agents'] = $this->db->query("SELECT COUNT(id) as total FROM customer WHERE role_id = 3")->row()->total;

			$data['total_trips'] = $this->db->query("SELECT COUNT(id) as total FROM outstation WHERE status > 0")->row()->total;
			$data['locals'] = $this->db->query("SELECT COUNT(id) as total FROM outstation WHERE type = 4 AND status > 0")->row()->total;
			$data['sights'] = $this->db->query("SELECT COUNT(id) as total FROM outstation WHERE type = 3 AND status > 0")->row()->total;
			$data['outs'] = $this->db->query("SELECT COUNT(id) as total FROM outstation WHERE type IN (1,2) AND status > 0")->row()->total;
