<?php  defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Location extends CI_Controller {
		...
		public function locality(){
			...
			$data['result'] = $this->Location_model->get_localities($city);
			$data['city'] = $this->Location_model->get_cities($state = false);
			if ($this->admin->getInfo()){
				$info = explode('--', $this->admin->getInfo());
				$data['info'] = $info[1];
				$data['info_type'] = $info[0];
			...
			$this->load->view('admin/location/locality_list',$data);
		}
		
		public function locality_add(){
			$data['city'] = $this->Location_model->get_cities($state = false);
			if($this->input->get('id')){
				$query = $this->Location_model->get_locality_by_id($this->input->get('id'));
				foreach($query->result() as $query){
				$data['id'] = $query->id;
			...
			$this->load->view('admin/location/locality_form',$data);
		}
		...
		public function add_locality(){
			...
			if($this->input->post('id')){
				$query = $this->Location_model->edit_locality();
			}
			else{
				$query = $this->Location_model->add_locality();
			}
			if($query){
					$this->session->set_userdata('info', "1--Successfully done");
				}
			...
			redirect('admin/location/locality');
		}
		}
		
		public function delete_locality(){
			$id = implode(',',$this->input->post('check_list'));
			$query = $this->Location_model->delete_locality($id);
			if($query){
					$this->session->set_userdata('info', "1--Successfully deleted");
				}
				else{
