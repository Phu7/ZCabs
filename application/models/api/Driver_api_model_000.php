...
class Driver_api_model extends CI_Model{
	...
	function manage_document(){
		...
		$this->load->helper('string');
		$query  = $this->db->query("UPDATE customer SET bank_name = '" . $this->db->escape_str($this->input->post('bank_name')) . "', bank_account = '" . $this->db->escape_str($this->input->post('bank_account')) . "', ifsc_code = '" . $this->db->escape_str($this->input->post('ifsc')) . "', address_proof = '" . $address_proof . "', vehicle_picture = '" . $vehicle_picture . "', insurance = '" . $insurance . "', license = '" . $license . "', bank_proof = '" . $bank_proof . "', vehicle_rc_book = '" . $vehicle_rc_book . "', account_holder_name = '" . $this->db->escape_str($this->input->post('account_holder_name')) . "', modified = NOW(), vehicle_type = '" . $this->db->escape_str($this->input->post('vehicle_type')) . "', vehicle_name = '" . $this->db->escape_str($this->input->post('vehicle_name')) . "', vehicle_number = '" . $this->db->escape_str($this->input->post('vehicle_number')) . "', vehicle_model = '" . $this->db->escape_str($this->input->post('vehicle_model')) . "', ip = '". $_SERVER['REMOTE_ADDR'] ."' WHERE id = '" . $this->input->post('user_id') . "'");
		return $query;
	}
