...
class Wd_req_model extends CI_Model{

	function get_wds(){
		$query = $this->db->query("SELECT wd.*, c.first_name, c.last_name, c.mobile, c.bank_name, c.account_holder_name, c.bank_account, c.ifsc_code, c.wallet FROM withdrawal_request wd LEFT JOIN customer c ON(wd.customer_id = c.id) Where wd.type='3' ");
		return $query;
	}
