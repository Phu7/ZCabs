		...
		<?php } echo $msg_data; ?> </div><?php } $this->admin->removeInfo();?>
            ...
            foreach($result->result() as $result){ ?>
              ...
              <td><input type="checkbox" name="check_list[]" value="<?php echo $result->id; ?>" /></td>
							...
							<td>
							<a class="btn btn-info btn-sm" href="<?php echo base_url(); ?>admin/driver/add?id=<?php echo $result->id; ?>">Edit</a><a class="btn btn-info btn-sm" href="<?php echo base_url(); ?>admin/driver/detail?id=<?php echo $result->id; ?>">Detail</a><?php echo ($result->status == '1') ? '<a class="btn btn-danger btn-sm" href="' . base_url().'admin/driver/update?id='.$result->id.'&status=2">Disable</a>':'<a class="btn btn-success btn-sm" href="' . base_url().'admin/driver/update?id='.$result->id.'&status=1">Enable</a>';?></td>
						</tr>
					  <?php } ?>
					  </tbody>
                    </table>
