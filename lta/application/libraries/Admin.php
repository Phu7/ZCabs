...
class Admin {
	...
	public function isType($id){
			$CI =& get_instance();
		$query = $CI->db->query("SELECT role_id FROM customer WHERE id = '" . (int)$id . "' AND status = '1'");
		if($query->num_rows()){
	...
	public function getAgentCommision(){
			$CI =& get_instance();
		$query = $CI->db->query("SELECT rate FROM commision WHERE id = '1'");
		if($query->num_rows()){
			return $query->row()->rate/100;
		} else{
			return 0;
		}
	}
